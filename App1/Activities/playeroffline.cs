...
#pragma warning restore CS0618 // El tipo o el miembro están obsoletos
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
          ...
          var  fondoblurreado = ImageHelper.CreateBlurredImageFromBitmap(this, 20, ((BitmapDrawable)d).Bitmap);
...
#pragma warning restore CS0618 // El tipo o el miembro están obsoletos
        ...
        public void escaneartodo() { 
            ...
            foreach (var audi in PlaylistsHelper.GetMedia(Constants.CachePath + "/downloaded.gr3d"))
                if (!File.Exists(audi.Path))
                    media.Add(audi);


            foreach (var vide in PlaylistsHelper.GetMedia(Constants.CachePath + "/downloaded.gr3d2"))
            ...
            try
            {
                if (indice <= nombreses.Count - 1 && indice >= 0)
                {
                    ...
                    if (Build.VERSION.SdkInt >= BuildVersionCodes.JellyBeanMr1)
                    {
                        ...
                        RunOnUiThread(() => fondo.SetImageBitmap(ImageHelper.CreateBlurredImageFromPortrait(this, 20, linkeses[indiceactual])));
                        animar5(fondo);
                    }
                    if (desdecache)
                    {
